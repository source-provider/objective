...
typedef struct global_State
{
    ...
    VM_SHUFFLE5(VM_SHUFFLE_SEMICOLON,
        ...
        gTtnameEnc<TString*> ttname[LUA_T_COUNT],       // names for basic types
        gTmnameEnc<TString*> tmname[TM_N]);             // array with tag-method names
...
struct extra_Space
{
    ...
    struct shared_t {
        ...
        uintptr_t all_threads;
    };
    const std::shared_ptr<shared_t> shared; //12
    void* weak_thread;    // 20
    uint16_t context_level;     // 24
    int unknowns[3];
    uintptr_t script_context; //40
    int ukn3; //44
    intptr_t faggot_context; // 48
    intptr_t idk;
    std::weak_ptr<uintptr_t> script_1;//56
};
...
struct lua_State
{
    ...
    VM_SHUFFLE6(VM_SHUFFLE_SEMICOLON,
        ...
        lGlobalEnc<global_State*> global,
