...
typedef struct TString
{
    ...
    tHashEnc<unsigned int> hash;
    tLenEnc<unsigned int> len;
...
typedef struct Udata
{
    ...
    uMetatableEnc<struct Table*> metatable;
...
typedef struct Proto
{
    ...
    VM_SHUFFLE5(VM_SHUFFLE_SEMICOLON,
    ...
    VM_SHUFFLE4(VM_SHUFFLE_SEMICOLON,
        pMember1Enc<TValue*> k,              // constants used by the function
        pMember1Enc<Instruction*>code,      // function bytecode
        pMember1Enc<struct Proto**> p,       // functions defined inside the function
    ...
    VM_SHUFFLE5(VM_SHUFFLE_SEMICOLON,
        pMember2Enc<uint8_t*> lineinfo,      // for each instruction, line number as a delta from baseline
        pMember2Enc<int*> abslineinfo,       // baseline line info, one entry for each 1<<linegaplog2 instructions; allocated after lineinfo
        pMember2Enc<struct LocVar*> locvars, // information about local variables
        pMember2Enc<TString**> upvalues,     // upvalue names
        pMember2Enc<TString*> source);

    pDebugNameEnc<TString*> debugname;
    pDebugInsnEnc<uint8_t*> debuginsn; // a copy of code[] array with just opcodes

    pTypeInfoEnc<uint8_t*> typeinfo;
...
typedef struct Closure
{
    ...
    union
    {
        struct
        {
            cFuncEnc<lua_CFunction> f;
            cContEnc<lua_Continuation> cont;
            cDebugNameEnc<const char*> debugname;
            TValue upvals[1];
        } c;

        struct
        {
            cFuncEnc<struct Proto*> p;
            TValue uprefs[1];
        } l;
    };
} Closure;
...
typedef struct Table
{
    ...
    VM_SHUFFLE4(VM_SHUFFLE_SEMICOLON,
        tMetatableEnc<struct Table*> metatable,
        tArrayEnc<TValue*> array,  // array part
        tNodeEnc<LuaNode*> node,
        GCObject* gclist);
} Table;
// clang-format on
...
#define luaO_nilobject reinterpret_cast<TValue*>(nilObjectAddress)
